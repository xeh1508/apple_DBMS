...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    uiOutput("user_table"),
    tags$footer(tags$i("A Sequal to Apple Sales Dashboard."), 
...
server <- function(input, output, session) {
  ...
  observeEvent(input$save, {
    ...
    showModal(modalDialog(
      title = "Success",
      paste0("Table ",paste0('top_store_',gsub(" ", "", input$prod_type_i))," has been successfully saved."),
      footer = modalButton("OK"), easyClose = TRUE ) )
    }
  }
  )
      ...
      box( title = 'Add Column', width = NULL, solidHeader = TRUE, status = "primary",
           ...
           wellPanel(
             ...
             selectInput(inputId = "add_col_type", label = "Add Column Type", 
                         choices = c("INT", "VARCHAR(255)","FLOAT","BOOLEAN")
             ),
  ...
  output$values <- renderUI({
    ...
    for (col in colnames(d)) {
      ...
      values[[col]] <- box(
          ...
          if (typ_i == 'BOOLEAN') {checkboxInput(inputId = paste0("value_", col), label = 'TRUE')}
          else if (typ_i == 'FLOAT' | typ_i == 'NUM' | typ_i == 'INT') {numericInput(inputId = paste0("value_", col), label = 'Value',value = 0)} 
          else {textInput(inputId = paste0("value_", col), label = 'Value')}
       )
    }
    values
  })
          ...
          sidebarLayout(
            sidebarPanel(
              ...
              selectInput(
                ...
                choices = setdiff(dbListTables(db),
                                  c("custs","iphones","order_items",
                                    "orders","prods","prods_i","store_sales",
                                    "stores"))
              ),
